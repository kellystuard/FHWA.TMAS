...
public sealed class StationDescriptionFormatter : ITrafficFormatter<StationDescription>
{
	...
	public StationDescription FromLine(in ReadOnlySpan<char> line)
	{
		...
		{
			StateCode = (Fips.State?)Strategies.ReadInt32(line, 2, 2) ?? throw new NullReferenceException(nameof(StationDescription.StateCode) + "is not optional"),
			StationID = Strategies.ReadLeftZeroFilledString(line, 4, 6) ?? throw new NullReferenceException(nameof(StationDescription.StationID) + "is not optional"),
			DirectionOfTravelCode = (DirectionOfTravel?)Strategies.ReadChar(line, 10) ?? throw new NullReferenceException(nameof(StationDescription.DirectionOfTravelCode) + "is not optional"),
			LaneOfTravelCode = Strategies.ReadNumber(line, 11) ?? throw new NullReferenceException(nameof(StationDescription.LaneOfTravelCode) + "is not optional"),
			YearOfData = Strategies.ReadInt32(line, 12, 4) ?? throw new NullReferenceException(nameof(StationDescription.YearOfData) + "is not optional"),
			FunctionalPurpose = (FunctionalPurpose?)Strategies.ReadChar(line, 16) ?? throw new NullReferenceException(nameof(StationDescription.FunctionalPurpose) + "is not optional"),
			FunctionalTypeCode = (FunctionalType?)Strategies.ReadChar(line, 17) ?? throw new NullReferenceException(nameof(StationDescription.FunctionalTypeCode) + "is not optional"),
			Lanes = Strategies.ReadNumber(line, 18) ?? throw new NullReferenceException(nameof(StationDescription.Lanes) + "is not optional"),
			StationUsedForTmas = Strategies.ReadChar(line, 19) == 'Y',
			LanesMonitoredForVolume = Strategies.ReadNumber(line, 20) ?? throw new NullReferenceException(nameof(StationDescription.LanesMonitoredForVolume) + "is not optional"),
			MethodOfVolumeCounting = (MethodOfVolumeCounting?)Strategies.ReadChar(line, 21) ?? throw new NullReferenceException(),
			LanesMonitoredForClassificationAndOrSpeed = Strategies.ReadNumber(line, 22) ?? throw new NullReferenceException(nameof(StationDescription.LanesMonitoredForClassificationAndOrSpeed) + "is not optional"),
			...
			LanesMonitoredForTruckWeight = Strategies.ReadNumber(line, 27) ?? throw new NullReferenceException(nameof(StationDescription.LanesMonitoredForTruckWeight) + "is not optional"),
